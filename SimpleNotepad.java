...
public class SimpleNotepad implements INotepad{
	...
	public void showInfo () {
		for (int i = 0; i < pages.length; i++) {
			if (pages[i] != null) {
				System.out.println("Page [" + i + "] info: " + pages[i].toString());
			} 
		}
	}
		...
		for (int i = 0; i < pages.length; i++) {
			if (pages[i] != null) {
				if (pages[i].searchWord(word)) {
					System.out.println("The word " + word + " contained on page " + i);
				}
			}
		}
	}
		...
		for (int i = 0; i < pages.length; i++) {
			if (pages[i] != null) {
				if (pages[i].containsDigits()) {
					System.out.println("Page " + i + " contains digits: " + pages[i].toString());
				}
			}
		}
